...
class BoxForm(ModelForm):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.initial['cash_sale'] = Box.total_efectivo() + Box.total_payments_ctas_collect()
        # self.initial['cash_credit'] = Box.total_credito()
        self.initial['sale_card'] = Box.total_tarjeta_yape_plin()
        self.initial['box_final'] = Box.total_efectivo() + Box.total_payments_ctas_collect() + Box.total_tarjeta_yape_plin()
        self.initial['hours_close'] = datetime.now().strftime('%H:%M')
    class Meta:
        model = Box
        fields = '__all__'
        widgets = {
            ...
            'hours_close': forms.TextInput(attrs={'type': 'time'}),
    ...
    class Meta:
        ...
        widgets = {
            'mobile': forms.TextInput(
                attrs={
                    'placeholder': 'Ingrese su número celular o teléfono',
                    'class': 'form-control',
                    'autocomplete': 'off'
                }
            ),
